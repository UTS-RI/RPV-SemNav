...
class ObstacleMap(BaseMap):
    ...
    def visualize(self, object_centroids: list = None) -> np.ndarray:
        """Visualizes the map.

        Args:
            object_centroids: Optional list of dicts with 'world_xy', 'label', 'score'
                keys. When provided, numbered markers are drawn at each centroid.
        """
        ...
        # Draw numbered detection markers
        if object_centroids:
            h, w = vis_img.shape[:2]
            for i, obj in enumerate(object_centroids):
                ...
                if world_xy is None:
                    continue
                px = self._xy_to_px(np.array(world_xy).reshape(1, 2))[0]
                # flip Y to match the cv2.flip above
                cx, cy = int(px[0]), h - 1 - int(px[1])
                if not (0 <= cx < w and 0 <= cy < h):
                    continue
                num_str = str(i + 1)
                radius = 16 if len(num_str) > 1 else 12
                font_scale = 0.42 if len(num_str) > 1 else 0.50
                cv2.circle(vis_img, (cx, cy), radius, (0, 0, 255), -1)  # red fill
                cv2.circle(vis_img, (cx, cy), radius, (255, 255, 255), 1)  # white border
                text_sz = cv2.getTextSize(num_str, cv2.FONT_HERSHEY_SIMPLEX, font_scale, 1)[0]
                tx = cx - text_sz[0] // 2
                ty = cy + text_sz[1] // 2
                cv2.putText(vis_img, num_str, (tx, ty),
