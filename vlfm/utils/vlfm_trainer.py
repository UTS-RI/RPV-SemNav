...
# For Torch 2.6 compatibility when loading checkpoints/weights using omegaconf
from omegaconf import DictConfig, ListConfig   
from omegaconf.base import ContainerMetadata, Metadata
from omegaconf.nodes import (
    AnyNode, IntegerNode, FloatNode, StringNode, BooleanNode, EnumNode
)

# For compatibility with habitat_baselines v0.3.X
from habitat_baselines.rl.ppo.evaluator import pause_envs  # Moved to a standalone function
...
class VLFMTrainer(PPOTrainer):
    ...
    def _load_checkpoint(self, checkpoint_path, *args, **kwargs):
        # Tell torch.serialization that it's safe to load the full dummy path as the origin is known to be safe
        # Therefore: using weights_only=False
    ...
    def _eval_checkpoint(
        ...
        if self.config.habitat_baselines.eval.should_load_ckpt:
            ...
            ckpt_dict = self._load_checkpoint(checkpoint_path, map_location="cpu")
        ...
        try:
            hidden_shape = self._agent.actor_critic.hidden_state_shape
        except NotImplementedError:
            hidden_shape = (1,1)    # VLFM is non-recurrent, so we can just use a dummy shape for the hidden states
        ...
        running_spl = 0.0
        hab_vis = HabitatVis()
        _ep_step_start = time.perf_counter()
        _ep_step_count = 0
        _ep_step_time = 0.0
        _global_step_time = 0.0
        ...
        while len(stats_episodes) < (number_of_eval_episodes * evals_per_ep) and self.envs.num_envs > 0:
            ...
            _step_elapsed = time.perf_counter() - _ep_step_start
            _ep_step_count += 1
            _ep_step_time += _step_elapsed
            _global_step_time += _step_elapsed
            _global_step_count += 1
            _ep_step_start = time.perf_counter()
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    ...
                    if episode_stats["success"] == 1:
                        num_successes += 1
                    running_spl += episode_stats.get("spl", 0.0)
                    num_total += 1
                    print(
                        f"Episode {num_total}: success={episode_stats['success']}, "
                        f"spl={episode_stats.get('spl', float('nan')):.3f}"
                    )
                    print(f"Success rate: {num_successes / num_total * 100:.2f}% ({num_successes} out of {num_total})")
                    print(f"Running SPL: {running_spl / num_total:.3f}")
                    if _ep_step_count > 0:
                        _ep_avg_ms = _ep_step_time / _ep_step_count * 1000
                        print(f"Episode avg step time: {_ep_avg_ms:.1f} ms ({_ep_step_count} steps)")
        ...
        if _global_step_count > 0:
            _global_avg_ms = _global_step_time / _global_step_count * 1000
            logger.info(f"Average step inference time: {_global_avg_ms:.1f} ms ({_global_step_count} total steps)")
