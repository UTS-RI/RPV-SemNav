...
def load_pointnav_policy(file_path: str) -> PointNavResNetTensorOutputPolicy:
    ...
    if HABITAT_BASELINES_AVAILABLE:
        ...
        if habitat_version == "0.1.5":
            ...
            # For new versions of torch, set weights_only=False to load the entire checkpoint dict, which includes the config needed to initialise the policy correctly.
            ckpt_dict = torch.load(file_path, map_location="cpu", weights_only=False) 

            # Patch old flat policy config to new 0.3.x Dict[str, PolicyConfig] format
            # Convert to plain unstructured container to bypass RLConfig struct validation
            # OmegaConf.create() produces a plain unstructured config with no type constraints at all. 
            # The to_container call strips all struct metadata first, then OmegaConf.create rebuilds it as a simple flexible 
            # dict-like object that accepts any assignment
            ckpt_config = ckpt_dict["config"]
            ckpt_config = OmegaConf.create(OmegaConf.to_container(ckpt_config, resolve=False))
            old_policy = ckpt_config.habitat_baselines.rl.policy
            ckpt_config.habitat_baselines.rl.policy = {"main_agent": old_policy}
