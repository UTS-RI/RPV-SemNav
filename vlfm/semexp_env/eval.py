...
# ---------------------------------------------------------------------------
#  Selective episode saving helpers
# ---------------------------------------------------------------------------

def _should_save_episode(ep_num: int) -> bool:
    """Return True for episodes 0-4 and every 57th episode thereafter."""
    if ep_num < 5:
        return True
    return (ep_num - 5) % 57 == 0


def _episode_dir(ep_num: int, ep_id: str) -> str:
    """Return the per-episode output directory path."""
    base = os.environ.get("VIDEO_DIR", "video_dir")
    dirname = f"ep{ep_num:04d}_{ep_id}"
    return os.path.join(base, dirname)


def _save_step_frame(ep_dir: str, step: int, frame: np.ndarray) -> None:
    """Save a single step frame as a PNG inside the episode directory."""
    os.makedirs(ep_dir, exist_ok=True)
    path = os.path.join(ep_dir, f"step_{step:04d}.png")
    cv2.imwrite(path, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))


def _save_episode_video(ep_dir: str, frames: List[np.ndarray], fps: int = 2) -> None:
    """Write a video at *fps* from the collected RGB frames."""
    if not frames:
        return
    os.makedirs(ep_dir, exist_ok=True)
...
def _save_results_txt(ep_dir: str, data: Dict) -> None:
    """Write a human-readable results.txt inside the episode directory."""
    os.makedirs(ep_dir, exist_ok=True)
    path = os.path.join(ep_dir, "results.txt")
...
def main() -> None:
    ...
    for ep_num in range(num_episodes):
        vis_imgs = []
        save_this_ep = _should_save_episode(ep_num) and "VIDEO_DIR" in os.environ
        ep_dir = ""  # will be set on step 0
        ...
        for step in range(args.max_episode_length):
            if step == 0:
                ...
                if save_this_ep:
                    ep_dir = _episode_dir(ep_num, ep_id)
                    print(f"  [logging] Saving episode {ep_num} to {ep_dir}")
            else:
            ...
            if "ZSOS_LOG_DIR" in os.environ and is_evaluated(ep_id, scene_id):
                ...
                if save_this_ep or "VIDEO_DIR" in os.environ:
                    frame = create_frame(policy_infos)
                    frame = add_text_to_image(frame, "Step: " + str(step), top=True)
                    vis_imgs.append(frame)

                    # Save per-step image for selected episodes
                    if save_this_ep:
                        _save_step_frame(ep_dir, step, frame)
            ...
            if done:
                ...
                # Selective per-episode logging: step images + 2FPS video + results.txt
                if save_this_ep:
                    try:
                        _save_episode_video(ep_dir, vis_imgs, fps=2)
                    except Exception as e:
                        print(f"Error generating episode video: {e}")
                    _save_results_txt(ep_dir, data)

                # Legacy full-speed video (all episodes when VIDEO_DIR is set)
